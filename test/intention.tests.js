...
describe('Intention', function () {
  ...
  var container = $('<div>'),
    ...
    nonResponsiveElm = $('<div id="getRidOfMe">'), responsiveElm1 = $('<div data-intent>');
  // basic in-* attrs
  ...
  describe('responsive: creating responsive functions', function () {
    ...
    it('should fire the axis event before the context event', function () {
      var intent = new Intention(), width = intent.responsive({
          contexts: [
            { name: 'mob' },
            { name: 'tab' }
          ],
  ...
  describe('is: test to see if a context name is in the .contexts property', function () {
    var intent = new Intention(), respond = intent.responsive([
        { name: 'foo' },
        { name: 'bar' }
      ]).respond;
    it('should confirm the applied context is current and others are not', function () {
  ...
  describe('_attrsToSpec: convert element\'s attrs to a responsive specification', function () {
    var mockAxes = {
        width: {
          contexts: [
            { name: 'mobile' },
    ...
    it('should match when many funcs are specified', function () {
      ...
      expect(intent._fillSpec(intent._attrsToSpec(elm[0].attributes, mockAxes))).to.eql({
        width: {
          mobile: {
            'class': 'foo',
            append: '#foo',
  ...
  describe('_resolveSpecs: from a list of names make object of changes', function () {
    it('should combine the classes and cascade to specified attr', function () {
      var intent = new Intention();
      expect(intent._resolveSpecs([
        {
          ctx: 'foo',
          axis: 'id1'
        },
  ...
  describe('_removeClasses: takes the spec and axes, figures out what classes to remove', function () {
    it('should return an array', function () {
      ...
      var result = intent._removeClasses({
          width: {
            tablet: { 'class': 'foo' },
            mobile: { 'class': 'foo bar' }
          },
          default: { base: { 'class': 'always' } },
          filler: { dud: { href: 'qux' } },
          orientation: {
            portrait: { class: 'portrait' },
            landscape: { class: 'landscape' }
  ...
  describe('_contextConfig: compiles a list of contexts to be applied and those that are not applied', function () {
  ...
  describe('_makeChanges: maniputate an element based on responsive specification', function () {
    var intent = new Intention(), elm = $('<div class="baz">'), axes = {
        id1: { current: 'foo' },
        id2: { current: null },
        __keys__: [
          'id1',
    ...
    it('should not have deleted a class that is not specified in the out contexts', function () {
  ...
  describe('jquery event', function () {
    it('should fire a jquery trigger event on every elm', function () {
      var intent = new Intention(), fire = false;
  ...
  describe('regex tests', function () {
    describe('full attr match', function () {
      ...
      it('should match on an abbreviated standard prefix', function () {
        expect(attrPattern.test('data-in-mobile-class')).to.equal(true);
      });
