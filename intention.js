...
(function (root, factory) {
  ...
  var Intention = function (params) {
    ...
    return intent;
      ...
      p = Intention.prototype;


  p.axis_test_pattern = new RegExp('^_[a-zA-Z0-9]');
  p.axis_match_pattern = new RegExp('^_([a-zA-Z0-9][_a-zA-Z0-9]*)');
  p.trim_pattern = new RegExp('^s+|s+$', 'g');

  p.responsive = function responsive(contexts, options) {
    ...
    this.on('_' + options.ID + ':', bind(function (e) {
      this.axes = this.contextualize(options.ID, e.context, this.axes);
      this.postContext();
    }, this));
    var axis = {
      ID: options.ID,
      ...
      respond: bind(this.responder(
        options.ID,
        options.contexts,
        options.matcher,
        options.measure), this)
    };
  ...
  p.postContext = function () {};

  p.responder = function (axisID, contexts, matcher, measure) {
    var currentContext;
  ...
  p.currentContexts = function (axes) {
    var contexts = [];
  ...
  p.contextualize= function (axisID, context, axes) {
